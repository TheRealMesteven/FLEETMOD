...
namespace FLEETMOD
{
    internal class Variables
    {
        ...
        public static Dictionary<int /*PlayerID*/ , /*ShipID*/ int> BotCrews;
        // Dictionary that stores <playerID,shipID> on host side, to quickly identify bots to correct ship.
        ...
        internal class Config : ModSettingsMenu
        {
            ...
            public override void Draw()
            {
                ...
                BeginVertical();
                Label("Bot Players");
                foreach (var k in BotCrews)
                {
                    BeginHorizontal();
                    Label($"id:{k.Key}");
                    PLPlayer pLPlayer = PLServer.Instance.GetPlayerFromPlayerID(k.Key);
                    if (pLPlayer != null)
                    {
                        Label($" name:{pLPlayer.GetPlayerName()}");

                        Label($" shipid:{k.Value}");
                        PLShipInfoBase shipInfoBase = PLEncounterManager.Instance.GetShipFromID(k.Value);
                        if (shipInfoBase != null)
                        {
